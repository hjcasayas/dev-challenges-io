...
const NavMobileTrigger: FunctionComponent<NavMobileTriggerProps> = ({
  ...
  if (open) {
    beforeStyles = `${beforeStyles} before:rotate-45 before:scale-x-[1.08] before:transition-transform before:duration-300 before:ease-in`;
    afterStyles = `${afterStyles} after:-rotate-45 after:scale-x-[1.08] after:transition-transform after:duration-300 after:ease-in`;
    middleStyles = `${middleStyles} scale-x-0 opacity-0 transition-all duration-300 ease-in`;
  }
